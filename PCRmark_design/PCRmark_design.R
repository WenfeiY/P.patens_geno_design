...
for(i in 1:nrow(PCRmarks_df2)){
  ...
  else{PCRmarks_df2$Feature_name[i]=paste(PCRmarks_df2$Feature_name[i],'.syn.R',sep='')}
}

write.table(PCRmarks_df, paste(path,'PP',chr_num,arm,"_PCRmark.bed",sep=''), sep = "\t", quote = FALSE, row.names = FALSE)
write.table(PCRmarks_df2, paste(path,'PP',chr_num,arm,"_PCRmark_editing.txt",sep=''), sep = "\t", quote = FALSE, row.names = FALSE)
